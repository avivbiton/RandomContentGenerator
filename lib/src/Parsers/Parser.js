...
function () {
  ...
  Parser.prototype.isDataValid = function (data) {
    ...
    for (var i = 0; i < parserKeys.length; i++) {
      ...
      if (index == -1 || typeof data[dataKeys[index]] !== typeof this[parserKeyName]) return false;
    }
  ...
  Parser.prototype.parseProperties = function (text) {
    ...
    for (var i = 0; i < this.properties.length; i++) {
      ...
      if (parser == null) throw new InvalidParserException_1.InvalidParserException(this.properties[i]);
  ...
  Parser.GetValidParser = function (data) {
    if (Array.isArray(data)) return new BasicParser_1.BasicParser(data);
    if (typeof data === "string") return new BasicParser_1.BasicParser([data]);
    var found = null;
